...
def evaluate_channel_quantization(model, data_loader, quant_layers, k=64):
    channel_metrics = []
    orig_size = 0
    total_size = 0
    ...
    # channel_metrics_2.sort()
    
    # for metric, name, c in channel_metrics_2[:(i)//10]:
    #     module = dict(model.named_modules())[name]
    #     weights = module.weight.data
    #     original = weights[c] if len(weights.shape) == 4 else weights[c].unsqueeze(0)
        
    #     dequant, _, _ = quantize_channel(original, bits=2)
    #     if len(weights.shape) == 4:
    #         module.weight.data[c] = dequant
    #     else:
    #         module.weight.data[c] = dequant.squeeze()
    ...
    #     total_size -= 0.0625 * original.numel()
    #     j += 1
    #     # Evaluate final accuracy
    #     if int(orig_size * 2 / total_size) > int(comp_ratio * 2):
    #         top1, top5 = evaluate_model(model, data_loader)
    ...
    # top1, top5 = evaluate_model(model, data_loader)
    # print(f"top 10 percentile channels quantized to 2-bit; acc: {top1, top5}, compression: {(comp_ratio):.1f}x\n")
...
# top1, top5 = evaluate_channel_quantization(model, val_loader, quant_layers, k=128)
...
def get_model():
    ...
    if args.model == 'mobilenet3':
        from torchvision.models import mobilenet_v3_large, MobileNet_V3_Large_Weights
        net = mobilenet_v3_large()
        checkpoint = torch.load('/home/arun/Desktop/Learning-Both-Weights-and-Connections-for-Efficient-NNs/mobilenet3_pruned.pth', weights_only=True)
    ...
    #     checkpoint = torch.load('/home/arun/Desktop/RL/amc/checkpoints/vgg16_pruned.pth', weights_only=True)
    #     net.load_state_dict(checkpoint['model_state_dict'])
        # net = net['model_state_dict']
    elif args.model == 'vgg16':
        from cifar10.models.vgg import VGG
        import torch.backends.cudnn as cudnn
        net = VGG('VGG16')
        net = torch.nn.DataParallel(net)
        cudnn.benchmark = True
        checkpoint = torch.load('/home/arun/Desktop/Learning-Both-Weights-and-Connections-for-Efficient-NNs/cifar10/checkpoint/pruned/v16_2.pth', weights_only=True)
        print(checkpoint.keys())
        net.load_state_dict(checkpoint)
    elif args.model == 'resnet50':
        from cifar10.models.resnet import ResNet50
        import torch.backends.cudnn as cudnn
        net = ResNet50()
        net = torch.nn.DataParallel(net)
        cudnn.benchmark = True
        checkpoint = torch.load('/home/arun/Desktop/Learning-Both-Weights-and-Connections-for-Efficient-NNs/cifar10/checkpoint/pruned/rn50_16.pth', weights_only=False)
        print(net)
        net.load_state_dict(checkpoint)
        ...
        net = torch.nn.DataParallel(net)
        cudnn.benchmark = True
        checkpoint = torch.load('/home/arun/Desktop/Learning-Both-Weights-and-Connections-for-Efficient-NNs/cifar10/checkpoint/pruned/rn18_2.pth', weights_only=True)
        net.load_state_dict(checkpoint)
...
if __name__ == '__main__':
    ...
    top1, top5 = evaluate_channel_quantization(net, val_loader, quant_layers = [torch.nn.modules.conv.Conv2d, torch.nn.modules.linear.Linear], k=50)
